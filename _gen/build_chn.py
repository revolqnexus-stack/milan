...
CSS = r"""
  ...
  * { box-sizing: border-box; margin: 0; padding: 0; }
  ...
  @media (min-width: 500px) {
    ...
    body.app-unlocked { display: flex; justify-content: center; }
    body.app-unlocked > *:not(#login-gate) { width: 100%; max-width: 480px; }
  }

  /* ── Login / device lock ── */
  body:not(.app-unlocked) { padding-bottom: 0; }
  body:not(.app-unlocked) > *:not(#login-gate) { display: none !important; }
  #login-gate {
    position: fixed; inset: 0; z-index: 9999;
    display: flex; align-items: center; justify-content: center;
    padding: 24px 16px;
    background:
      radial-gradient(ellipse at 20% 0%, rgba(26,115,232,0.18), transparent 50%),
      radial-gradient(ellipse at 80% 100%, rgba(52,168,83,0.12), transparent 45%),
      var(--bg);
  }
  body.app-unlocked #login-gate { display: none !important; }
  .login-card {
  ...
  .login-label {
    ...
    color: var(--subtext); margin: 0 0 6px 2px;
  }
  .login-input {
  ...
  .login-note {
    margin-top: 14px; font-size: 11px; color: var(--subtext);
